...
describe Podcast::RssItem do
  before(:all) { @podcast_xml = File.read(File.join(Rails.root, 'spec/files/podcast_xml', 'itunes.xml')) }
  let!(:parent_channel) { Podcast::RssChannel.new(@podcast_xml) }
  let(:spamy) { Podcast::RssChannel.new(@spamy_content).items.first }
  subject { parent_channel.items.first }

  describe '.new(parent_channel, rss_item_obj)' do
    it "requires a Podcast::RssChannel" do
      lambda { Podcast::RssItem.new(nil, nil) }.should raise_error(ArgumentError, 'Podcast::RssChannel required')
      lambda { Podcast::RssItem.new('NOT Podcast::RssChannel', nil)}.should raise_error(ArgumentError, 'Podcast::RssChannel required')
    end

    it "requires a RSS::Rss::Channel::Item" do
      lambda { Podcast::RssItem.new( parent_channel, nil ) }.should raise_error(ArgumentError, 'RSS::Rss::Channel::Item required')
      lambda { Podcast::RssItem.new( parent_channel, 'NOT RSS::Rss::Channel::Item')}.should raise_error(ArgumentError, 'RSS::Rss::Channel::Item required')
    end

    it "builds from proper arguments" do
      lambda { Podcast::RssItem.new( parent_channel, parent_channel.native_rss_items.first ) }.should_not raise_error
    end
  end
