...
describe Podcast::RssChannel do
  before(:all) { @podcast_xml = File.read(File.join(Rails.root, 'spec/files/podcast_xml', 'itunes.xml')) }
  subject { Podcast::RssChannel.new(@podcast_xml) }
  let(:spamy) { Podcast::RssChannel.new(@spamy_content) }

  describe '.new(xml_str)' do
    it "requires an xml string" do
      lambda { Podcast::RssChannel.new }.should raise_error(ArgumentError)
      lambda { Podcast::RssChannel.new('') }.should raise_error(ArgumentError)
      lambda { Podcast::RssChannel.new('<not xml>')}.should raise_error(RSS::NotWellFormedError)
      lambda { Podcast::RssChannel.new(@podcast_xml) }.should_not raise_error
    end
  end
  ...
  describe 'items' do
    it "is an array of Podcast::RssItem" do
      subject.items.should be_a Array
      subject.items.first.should be_a Podcast::RssItem
    end
  end
